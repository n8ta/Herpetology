module Quiz
  class QuizController < ApplicationController
    ...
    def pick_region
      ...
      @regions = Region.countries.includes(:taxons).select {|reg| reg.taxons.species.includes(:photos).where(root_taxon_id: @taxon.id).select {|sp| sp.photos.length > 0 }.length > 5}
    end
